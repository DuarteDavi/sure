...
EMAIL = "vavamij504@discrip.com"
PASSWORD = "123456"

# Horários de operação (24h)
LOGIN_HOUR_RANGE = (6, 23)   # Login entre 6h e 23h
LOGOFF_HOUR_RANGE = (2, 3)   # Logoff entre 2h e 3h (madrugada)

# Delays e tentativas
SCRAPE_DELAY_RANGE = (50, 98)   # Delay entre ciclos de scraping (segundos)
LOGIN_ATTEMPTS = 3
SLOW_MO_RANGE = (100, 300)      # ms
INITIAL_DELAY_RANGE = (2.0, 5.0) # segundos

# Pausas longas simulando inatividade
LONG_PAUSE_CHANCE = 0.1         # 10% de chance de pausa longa
LONG_PAUSE_RANGE = (600, 1800)  # 10-30min
...
def is_logoff_time():
    now = datetime.now()
    return LOGOFF_HOUR_RANGE[0] <= now.hour <= LOGOFF_HOUR_RANGE[1]

def is_login_time():
    now = datetime.now()
    return LOGIN_HOUR_RANGE[0] <= now.hour <= LOGIN_HOUR_RANGE[1]

def wait_until_login():
    now = datetime.now()
    if now.hour < LOGIN_HOUR_RANGE[0]:
        target = now.replace(hour=LOGIN_HOUR_RANGE[0], minute=0, second=0, microsecond=0)
    else:
        target = (now + timedelta(days=1)).replace(hour=LOGIN_HOUR_RANGE[0], minute=0, second=0, microsecond=0)
    wait_seconds = (target - now).total_seconds()
    print(f"[BOT] Fora do horário de operação. Aguardando até {target.strftime('%H:%M')} ({wait_seconds/60:.1f} min)")
    time.sleep(wait_seconds)

def maybe_long_pause():
    if random.random() < LONG_PAUSE_CHANCE:
        pause = random.randint(*LONG_PAUSE_RANGE)
        print(f"[BOT] Pausa longa simulando inatividade: {pause//60} min")
        time.sleep(pause)
...
async def start_scraping(app):
    ...
    while True:
        if is_logoff_time():
            print("[BOT] Horário de logoff. Encerrando sessão e aguardando próximo horário de login...")
            await asyncio.sleep(5)  # Pequeno delay antes de sair
            wait_until_login()
            continue
        try:
            async with async_playwright() as p:
                ...
                context = await p.chromium.launch_persistent_context(
                    ...
                    slow_mo=random.randint(*SLOW_MO_RANGE),
                )
                ...
                for attempt in range(1, LOGIN_ATTEMPTS + 1):
                    if is_logoff_time():
                        print("[BOT] Horário de logoff detectado durante login. Abortando tentativa...")
                        break
                    print(f"Tentativa de login {attempt}/{LOGIN_ATTEMPTS}")
                ...
                while is_login_time():
                    try:
                        ...
                        await asyncio.sleep(random.randint(*SCRAPE_DELAY_RANGE) + random.uniform(0, 8))
                        maybe_long_pause()
                        if is_logoff_time():
                            print("[BOT] Horário de logoff chegou. Encerrando sessão...")
                            break
                    except Exception as e:
                        print(f"Error in scraping loop: {str(e)}")
                        await asyncio.sleep(random.randint(*SCRAPE_DELAY_RANGE) + random.uniform(0, 8))
                        maybe_long_pause()
                print("[BOT] Finalizando contexto Playwright para novo ciclo.")
                await context.close()
        except Exception as e:
            print(f"[BOT] Erro geral: {e}. Reiniciando ciclo após delay...")
